...
def common_options(func):
    options = [
        ...
        click.option("--model-dir", "-m", required=False, default=None, help="Directory where the model is stored."),
        click.option("--cutoff", "-c", required=False, default=None, help="Cutoff threshold (e.g. probability or value)."),
        click.option("--direction", "-d", required=False, default=None, help="Direction of processing (e.g. forward or backward)."),
        click.option("--parameters", "-p", required=False, default=None, help="Additional model parameters as a string."),
...
@click.option("--override-dir", required=False, is_flag=True, default=False, help="Path to the output model dir.")
def predict(input_file, model_dir, cutoff, direction, parameters, clean, flush, anonymize, output_dir, override_dir):
  logger.info("[#ff69b4]Running the setup pipeline to preprocess the input data[/]")
