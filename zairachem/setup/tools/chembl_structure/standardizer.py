...
_normalizer = rdMolStandardize.NormalizerFromData(_normalization_transforms, _normalizer_params)
    ...
    if atom.GetAtomicNum() == 1 and atom.GetDegree() == 1 and not atom.GetIsotope():
      ...
      if bnd.GetBondDir() in (
        ...
        if nbr.GetChiralTag() in (
          ...
          if nbr.GetExplicitValence() > Chem.GetPeriodicTable().GetDefaultValence(
            nbr.GetAtomicNum()
          ):
            preserve = True
          else:
            ringBonds = [b for b in nbr.GetBonds() if m.GetRingInfo().NumBondRings(b.GetIdx())]
            if len(ringBonds) >= 3:
              preserve = True
      if preserve:
...
def get_fragment_parent_mol(m, check_exclusion=False, neutralize=False, verbose=False):
  ...
  for i, v in enumerate(keep):
    ...
    # want to risk a full sanitiza-tion:
...
def get_parent_mol_from_smiles(smiles: str, get_smiles: bool = False, sanitize: bool = False):
  """
...
def standardize_molblock_from_smiles(smiles: str, get_smiles: bool = False, sanitize: bool = False):
  """
